...
我们挑选了 3 个非常有代表性的上游项目，提取了它们的核心工作流，并重构成通用的 Skill 包：
...
为了让这些工作流更易用，我们在每套包里都设计了一个非常重要的能力：**Mini Skill**（如 `mini-openspec`）。
...
如果你也在寻找一套不被单一平台绑定、又能让 Agent 遵循良好工程纪律的 Workflow 库，或者想低成本体验 OpenSpec、Superpowers 和 GStack 的核心方法，欢迎来试试这个项目。
...
We selected 3 highly representative upstream projects, extracted their core workflows, and refactored them into universal Skill packages:
...
To make these workflows easier to use, we designed a crucial feature in each package: the **Mini Skill** (e.g., `mini-openspec`).
...
If you are looking for a workflow library that isn't locked to a single platform while keeping your Agent disciplined, or if you want to experience the core methods of OpenSpec, Superpowers, and GStack at low cost, welcome to try this project.
